...
impl GetterRpcClient {
    ...
    async fn send_request(&self, method: &str, params: Value) -> Result<Value, RpcError> {
        ...
        let json_body = serde_json::to_string(&rpc_request).map_err(|e| RpcError {
            message: format!("JSON serialization error: {}", e),
        })?;

        let uri: Uri = self.server_url.parse().map_err(|e| RpcError {
        ...
        let resp = self.client.request(req).await.map_err(|e| RpcError {
        ...
        let rpc_response: RpcResponse =
            serde_json::from_slice(&body_bytes).map_err(|e| RpcError {
    ...
    pub async fn add_app(&self, app_data: serde_json::Value) -> Result<(), RpcError> {
        ...
        let hub_uuid = app_data["hub_uuid"]
            .as_str()
            .unwrap_or("github")
            .to_string();
    ...
    pub async fn get_app_status(&self, app_id: String) -> Result<Option<AppStatusInfo>, RpcError> {
        ...
        if result.is_null() {
            ...
            let status = serde_json::from_value(result).map_err(|e| RpcError {
                message: format!("Response parse error: {}", e),
            })?;
            Ok(Some(status))
        }
