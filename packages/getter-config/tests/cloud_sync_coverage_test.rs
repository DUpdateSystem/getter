...
async fn test_cloud_sync_sync_to_repo_method() {
    ...
    assert_eq!(
        mapping.get("fd9b2602-62c5-4d55-bd1e-0d6537714ca0").unwrap(),
        "github"
    );
}
...
async fn test_repository_manager_sync_repository() {
    ...
    repo_manager
        .set_repository_priority("test-repo", 100)
        .unwrap();
    let repos = repo_manager.get_repositories();
...
fn test_repository_find_app_in_repositories() {
    ...
    repo_manager
        .add_repository("repo1".to_string(), None, 10)
        .unwrap();
    repo_manager
        .add_repository("repo2".to_string(), None, 20)
        .unwrap();
...
fn test_create_app_identifier() {
    ...
    cloud_sync
        .uuid_to_name_map
        .insert("app-uuid".to_string(), "myapp".to_string());
    cloud_sync
        .uuid_to_name_map
        .insert("hub-uuid".to_string(), "github".to_string());
