use getter_config::{AppRegistry, repository::RepositoryManager, cloud_sync::CloudSync};
use std::fs;
...
async fn test_sync_multiple_repositories() {
    ...
    assert_eq!(shared_config.metadata.get("version").unwrap(), "local-1.0.0");
...
async fn test_repository_priority_override() {
    ...
    repo_manager.add_repository("low".to_string(), None, 10).unwrap();
    repo_manager.add_repository("medium".to_string(), None, 50).unwrap();
    repo_manager.add_repository("high".to_string(), None, 100).unwrap();
    ...
    assert_eq!(merged_config.metadata.get("author").unwrap(), "Original Author"); // preserved from low
    ...
    assert_eq!(merged_config.metadata.get("priority_override").unwrap(), true); // from high
...
async fn test_disabled_repository_ignored() {
    ...
    repo_manager.add_repository("active".to_string(), None, 50).unwrap();
    repo_manager.add_repository("disabled".to_string(), None, 100).unwrap();
...
async fn test_concurrent_repository_sync() {
    ...
    for (repo_name, _) in test_configs.iter() {
        ...
        let task = tokio::spawn(async move {
            ...
            for hub in &cloud_config.hub_config_list {
                ...
                cloud_sync.uuid_to_name_map.insert(hub.uuid.clone(), hub_id.clone());
    ...
    println!("Concurrent sync of {} repositories took: {:?}", results.len(), elapsed);
...
async fn test_dynamic_priority_change() {
    ...
    repo_manager.add_repository("repo_a".to_string(), None, 50).unwrap();
    repo_manager.add_repository("repo_b".to_string(), None, 50).unwrap();
...
async fn test_repository_removal() {
    ...
    repo_manager.add_repository("keep1".to_string(), None, 10).unwrap();
    repo_manager.add_repository("remove".to_string(), None, 50).unwrap();
    repo_manager.add_repository("keep2".to_string(), None, 30).unwrap();
...
async fn test_many_repositories_performance() {
    ...
    println!("Created {} repositories with {} total apps in: {:?}",
             num_repos, num_repos * apps_per_repo, setup_time);
