...
fn test_real_cloud_config_parsing() {
    ...
    assert!(test_data_path.exists(), "Test data file not found at {:?}", test_data_path);
    ...
    let upgradeall = cloud_config.app_config_list.iter()
    ...
    assert_eq!(upgradeall.base_hub_uuid, "fd9b2602-62c5-4d55-bd1e-0d6537714ca0");
    
    let apkgrabber = cloud_config.app_config_list.iter()
    ...
    let github_hub = cloud_config.hub_config_list.iter()
    ...
    let google_play_hub = cloud_config.hub_config_list.iter()
    ...
    let android_apps: Vec<_> = cloud_config.app_config_list.iter()
        .filter(|app| app.info.extra_map.contains_key("android_app_package"))
        .collect();
    println!("Found {} Android apps with package names", android_apps.len());
...
async fn test_multi_repo_with_real_data() {
    ...
    repo_manager.add_repository(
        "main".to_string(),
        Some("https://raw.githubusercontent.com/DUpdateSystem/getter/master/cloud_config.json".to_string()),
        0
    ).unwrap();
    ...
    assert_eq!(app_config.metadata.get("community_version").unwrap(), "2.0.0");
    assert_eq!(app_config.metadata.get("community_notes").unwrap(), "Enhanced by community");
...
fn test_uuid_mapping_with_real_data() {
    ...
    for (uuid, expected_name) in &known_mappings[2..] {
        let hub = cloud_config.hub_config_list.iter()
            .find(|h| h.uuid == *uuid)
            .expect(&format!("Hub with UUID {} should exist", uuid));
        
        let (hub_id, _) = cloud_sync.convert_hub_item(hub);
        assert_eq!(hub_id, *expected_name, "Hub {} should map to {}", uuid, expected_name);
    }
    
    // Verify app conversions
    for (uuid, expected_name) in &known_mappings[..2] {
        let app = cloud_config.app_config_list.iter()
            .find(|a| a.uuid == *uuid)
            .expect(&format!("App with UUID {} should exist", uuid));
        
        let (app_id, _) = cloud_sync.convert_app_item(app);
        assert_eq!(app_id, *expected_name, "App {} should map to {}", uuid, expected_name);
    }
}
...
fn test_app_categories_from_real_data() {
    ...
    // Group apps by hub
    let mut apps_by_hub: std::collections::HashMap<String, Vec<String>> = std::collections::HashMap::new();
    
    for app in &cloud_config.app_config_list {
        let hub_name = cloud_config.hub_config_list.iter()
            .find(|h| h.uuid == app.base_hub_uuid)
            .map(|h| h.info.hub_name.clone())
            .unwrap_or_else(|| "Unknown".to_string());
        
        apps_by_hub.entry(hub_name).or_insert_with(Vec::new).push(app.info.name.clone());
    }
