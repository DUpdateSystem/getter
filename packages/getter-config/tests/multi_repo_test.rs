...
async fn test_cloud_sync_with_real_data() {
    ...
    assert_eq!(upgradeall.info.extra_map.get("android_app_package"), Some(&"net.xzos.upgradeall".to_string()));
...
async fn test_repository_overlay() {
    ...
    // Setup repository manager
    let mut repo_manager = RepositoryManager::new(data_path).unwrap();
    repo_manager.add_repository("main".to_string(), None, 0).unwrap();
    repo_manager.add_repository("community".to_string(), None, 50).unwrap();
    ...
    assert_eq!(app_config.metadata.get("description").unwrap(), "Main repo version"); // Main repo
...
async fn test_multiple_cloud_sources() {
    ...
    let has_android_apps = main_config.app_config_list.iter()
    ...
    let has_dev_tools = community_config.app_config_list.iter()
    ...
    let docker_hub = community_config.hub_config_list.iter()
...
fn test_uuid_to_name_mapping() {
    ...
    let cloud_app = getter_config::cloud_sync::CloudAppItem {
        ...
        info: getter_config::cloud_sync::CloudAppInfo {
            ...
            extra_map: std::collections::HashMap::from([
                ("android_app_package".to_string(), "net.xzos.upgradeall".to_string()),
            ]),
        },
    };
    ...
    assert_eq!(app_config.metadata.get("uuid").unwrap(), "f27f71e1-d7a1-4fd1-bbcc-9744380611a1");
    assert_eq!(app_config.metadata.get("base_hub_uuid").unwrap(), "fd9b2602-62c5-4d55-bd1e-0d6537714ca0");
    assert_eq!(app_config.metadata.get("android_app_package").unwrap(), "net.xzos.upgradeall");
}
