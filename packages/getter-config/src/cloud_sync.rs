...
impl CloudSync {
    ...
    pub fn convert_app_item(&self, cloud_app: &CloudAppItem) -> (String, AppConfig) {
        ...
        let app_id = self.uuid_to_name_map.get(&cloud_app.uuid)
            .cloned()
            .unwrap_or_else(|| {
        ...
        metadata.insert("base_hub_uuid".to_string(), serde_json::json!(cloud_app.base_hub_uuid));
        metadata.insert("url".to_string(), serde_json::json!(cloud_app.info.url));
        metadata.insert("base_version".to_string(), serde_json::json!(cloud_app.base_version));
        metadata.insert("config_version".to_string(), serde_json::json!(cloud_app.config_version));
    ...
    pub fn convert_hub_item(&self, cloud_hub: &CloudHubItem) -> (String, HubConfig) {
        ...
        let hub_id = self.uuid_to_name_map.get(&cloud_hub.uuid)
            .cloned()
            .unwrap_or_else(|| {
                // Convert name to lowercase and replace spaces with hyphens
        ...
        config_map.insert("base_version".to_string(), serde_json::json!(cloud_hub.base_version));
        config_map.insert("config_version".to_string(), serde_json::json!(cloud_hub.config_version));
        config_map.insert("hub_icon_url".to_string(), serde_json::json!(cloud_hub.info.hub_icon_url));
        config_map.insert("target_check_api".to_string(), serde_json::json!(cloud_hub.target_check_api));
        config_map.insert("api_keywords".to_string(), serde_json::json!(cloud_hub.api_keywords));
        config_map.insert("app_url_templates".to_string(), serde_json::json!(cloud_hub.app_url_templates));
    ...
    pub async fn sync_to_repo(&mut self, repo_path: &Path) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    pub fn create_app_identifier(&self, cloud_app: &CloudAppItem) -> String {
        ...
        let hub_id = self.uuid_to_name_map.get(&cloud_app.base_hub_uuid)
            .cloned()
...
impl crate::app_registry::AppRegistry {
    ...
    pub async fn sync_from_cloud(&mut self, cloud_url: &str) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    pub async fn import_cloud_apps(&mut self, cloud_url: &str) -> Result<Vec<String>, Box<dyn Error + Send + Sync>> {
...
mod tests {
    ...
    fn test_cloud_sync_conversion() {
        let cloud_app = CloudAppItem {
            ...
            info: CloudAppInfo {
                ...
                extra_map: HashMap::from([
                    ("android_app_package".to_string(), "com.example.app".to_string()),
                ]),
            },
        };
        ...
        assert_eq!(app_config.metadata.get("android_app_package").unwrap(), "com.example.app");
    }
}