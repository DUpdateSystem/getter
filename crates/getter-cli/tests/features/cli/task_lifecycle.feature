@getter-cli @debug-fake-task
Feature: Offline fake task lifecycle
  Scenario: Runtime script exercises ADR-0011 in-memory remove and clean
    Given an initialized getter data directory
    And a runtime script that submits completes removes and cleans a task
    When I run getter runtime script for that script
    Then the command succeeds
    And the output is valid JSON
    And the runtime script output removes the completed task
  ...
  Scenario: User polls offline task events with cursor and limit
    ...
    When I run getter debug fake-task run for the remembered task
    Then the command succeeds
    When I run getter debug fake-task events after 0 limit 2
    Then the command succeeds
    And the task events output contains 2 events and has more events
    And I remember the next event cursor
    When I run getter debug fake-task events after the remembered cursor limit 10
    Then the command succeeds
  ...
  Scenario: User records an offline install handoff result
    ...
    When I run getter debug fake-task install-result "succeeded" for the remembered handoff
    Then the command succeeds
  ...
  Scenario: User cannot record getter-created requested state as an install result
    ...
    Then the command succeeds
    And I remember the install handoff id
    When I run getter debug fake-task install-result "requested" for the remembered handoff
    Then the command fails with a CLI usage error

  Scenario: User cannot poll task events with a zero limit
    Given an initialized getter data directory
    When I run getter debug fake-task events after 0 limit 0
    Then the command fails with a CLI usage error
